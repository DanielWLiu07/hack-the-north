...
- **room.yaml (a human call):** objects outside every zone box — the floor — have no zone.
  ...
  **As built 2026-09-19 (perception/segment):** `scan_into` does not rewrite room.yaml. A
  hallway capture whose fused cloud is not on the desk gets an in-memory `DEFAULT_FLOOR_ZONE`
  for that scan and `zones/floor/**` is not applied, so a can becomes a file. Pinning a floor
  box in room.yaml is still the human call for mixed desk+floor rooms. Colourful packs under
  the height seed (cap_0015's flat wrapper at 1.3 m) seed on HSV saturation vs this capture's
  blank-floor p90; empty grey floors stay empty. Pack-seeded masks grow from the colourful
  core while they stay majority-saturated, so the instance is the bag, not the grey stereo halo.
...
- **perception/segment:** discarded masks/clusters are now `room-observations` rows with
  `object_id: null` and `rejected_reason` set (D14). Out-of-zone `keep()` drops still are not.
...
| D14 | **The real pipeline indexes no rejected clusters.** `cluster.py` counts its rejects and drops them; `merge` sets `rejected_reason: null` on every row. docs/11 Gap 1 is explicit that the discard pile goes to ES — it's the "honest mess". Today only the fake produces rejected rows, so the capture page will show none on real data. | **Resolved 2026-09-19 (perception/segment):** `cluster.cluster` / `segment.lift` / `segment.run` fill a caller-owned `rejects=` list (`too_small`, `plane_fragment`, `no_depth`, `roomignore:<label>`). `pipeline.capture_docs` indexes them with `object_id: null` and a unique millisecond each (TSDS). Out-of-zone is not a discard-pile row. |
...
| D49 | **One frame, two names, and the last hop refused.** The wire token everywhere on our side is `world_z_up` (`bridge/contract.FRAME`, `gitspace.plan/1`, web, docs/31 §4, ANDREW-HANDOFF §5), and Andrew's edge agrees since `7a31596` (`DANIEL_FRAME = "world_z_up"`, validated and preserved). But `robot/frames.py` had `ROOM_FRAME = "canonical_world_z_up"` — his OLD hard-coded constant — so `robot/adapter.py` refused every point: *"coordinate_frame is 'world_z_up'; only 'canonical_world_z_up' is accepted — a frame is never guessed"*. The refusal was right; the name was stale. | **Fixed: robot, 17:18Z** — `ROOM_FRAME = "world_z_up"`, with the old name accepted as an input alias only. A name alias, never a geometric one: both mean X fwd, Y left, Z up, metres. **Open:** the running `--sim` adapter still holds the old constant (started 14:39Z) and needs a restart before the beat-5 point succeeds. |
| D50 | **`odom_residual` is a SIMULATED signal, and the "why was this diff wrong" beat leans on it.** It stopped at 07:07:11Z with 6,854 docs while pitch / tilt_rate / encoders / currents ran on to 17:59:48Z with ~280,000 each. Not a network drop and not a mapping limit: until 07:07 every signal arrives in equal counts (one source emitting all eight — `robot/sim.py:65`, and `robot/telemetry.py`'s FakeRobot), and from 07:45 the rate is ~20,000 per 15 min with **zero** `odom_residual` (the real robot). `robot/bbos.py` (IMU + encoders from bbos) and `robot/balance_source.py` (BB's balance packets) never carry it. | **Answered: cloud.** Nothing is broken; the signal was only ever the sim's. `roomctl/why.py` degrades silently (the odometry finding just never fires), so on real captures the beat quietly loses that evidence. **Open, a decision:** either compute a residual on the robot (encoder odometry vs the SLAM pose, both already in `bbos.py`) and publish it as `odom_residual`, or have `why.py` say "odometry was not recorded for this window" so the absence is visible rather than silent. |
