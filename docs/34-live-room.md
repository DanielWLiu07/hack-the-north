...
| **`zone [--zone table] [--dir MAP] [name]`** | yes | measure the table: the largest horizontal surface between 0.45 and 1.15 m in the robot's map → `room.yaml` `zones.table` of the instance (measured 2026-09-20: 0.73 m, 1.98 m²). What `add --name` and the scan look inside | `0` · `1` no such surface (is the table in view?) |
