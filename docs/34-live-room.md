...
**2 · Robot server up** — only if step 1 did not say `ANSWERS`. Since 2026-09-19 `robot.server` and the adapter are
systemd user units on the robot ([`RUNBOOK` §9](../robot/RUNBOOK.md)): they come back by themselves ~25 s after a boot
(proven on an unplanned reboot the same night). So after a power cycle, wait half a minute and run step 1 again. If it
still does not answer — or the robot moved to another network — `--start` syncs the code, rewrites the robot's
`ROBOT_ALLOW` to this laptop's current address, and restarts both units through systemd:
```bash
./scripts/push_to_pi.sh bracketbot@<PI_HOST> --start --adapter
```
```
...
| **`zone [--zone table] [--keep] [--dir MAP] [name]`** | yes | measure the table: the largest horizontal surface between 0.45 and 1.15 m in the robot's map → `room.yaml` `zones.table` of the instance (measured 2026-09-20: 0.73 m, 1.98 m²). **Replaces** the zones already there (a `room.yaml` copied from `room.git` carries another room's `desk` and `shelf`; overlapping zones cut an object in half at the boundary and it names nothing — measured) and removes their `zones/<name>/` objects; `--keep` adds instead. **Run it before the first `add --name`** of an instance: a name is given to a NEW object, so an instance already scanned with the wrong zones keeps its `unknown_…` identities | `0` · `1` no such surface (is the table in view?) |
| **`add --name …`** | yes | `add`, and then perception's own path before the commit: `bb_source.scan_into_bb` puts the map's objects inside the zones into `zones/<zone>/<name>.yaml`, named where the segmenter recognises them in the head frame of the capture `add` just took (placed by the capture's own SLAM pose): `laptop_73b0`, else `unknown_…`. Prints `head frame sees N of M zone objects` — 0 means turn the robot to face the zone | as `add` |
| **`explore [--spin K] [--minutes M] [--go] …`** | drives | `scripts/room_explore.py`: turn on the spot through K headings (a full `add` at each, the robot still) and/or sweep the mapped area in lanes with a map-only `add` every 30 s while driving; plan only without `--go`, and `--go` still asks for a typed `clear`. Guards: SLAM localized, no other job, area ≥ 1 m², robot inside it, drive daemon fresh, time and distance ceilings, halt on Ctrl-C / lost contact / a map reset. Pose published live to `/robot`. First real run: the turn alone, a person beside the robot | `0` · `1` refused or a fault |
