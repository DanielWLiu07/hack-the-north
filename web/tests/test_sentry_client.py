...
@pytest.mark.parametrize("post, expect", [
    (httpx.Response(404, json={}), "this Sentry plan or version has no autofix endpoint (HTTP 404)"),
...
def test_seer_stumped_on_the_start_call(post, expect):
    seen = []
    c = client([httpx.Response(200, json=ISSUE), post], seen=seen)
    out = run(c.ask_seer("cap_0004"))
    assert out["state"] == "stumped" and expect in out["reason"], out["reason"]
    assert out["issue"]["short_id"] == "GITSPACE-3" and out["verdict"] is None
    assert [r.method for r in seen] == ["GET", "POST"], "a POST starts a billed run: it must never be retried"
    assert seen[1].url.path.endswith("/issues/7741490949/autofix/")
...
def test_seer_verdict_after_polling():
    ...
    c = client([httpx.Response(200, json=ISSUE), httpx.Response(202, json={"run_id": 42}),
                httpx.Response(200, json={"autofix": {"run_id": 42, "status": "PROCESSING"}}), httpx.Response(200, json=done)], seen=seen, slept=slept)
    ...
    assert out["run"] == {"run_id": 42, "status": "COMPLETED"} and slept == [2.0]
    assert b"20 telemetry breadcrumbs" in seen[1].content


def test_seer_stumped_when_the_run_says_nothing_readable_or_never_finishes():
    c = client([httpx.Response(200, json=ISSUE), httpx.Response(202, json={}), httpx.Response(200, json={"autofix": {"status": "COMPLETED", "steps": []}})])
    out = run(c.ask_seer("cap_0004"))
    assert out["state"] == "stumped" and "nothing readable" in out["reason"] and "status, steps" in out["reason"]
    c = client([httpx.Response(200, json=ISSUE), httpx.Response(202, json={})] + [httpx.Response(200, json={"autofix": {"status": "PROCESSING"}})] * 4)
