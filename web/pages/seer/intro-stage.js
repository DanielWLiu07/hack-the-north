...
export function createIntroStage(canvas, reduced) {
  ...
  style.textContent = `
    ...
    .seer-room > .sitenav { position:relative;z-index:3; }
  ...
  const status = document.createElement('span'); status.className = 'seer-state'; status.setAttribute('role', 'status'); status.textContent = 'Ready';
  stage.querySelector('.bandnav')?.prepend(status);
  document.body.classList.add('seer-room');
  const skip = document.createElement('button'); skip.className = 'seer-intro-skip'; skip.textContent = 'Skip intro';
  document.body.append(skip); document.body.classList.add('seer-intro');
  ...
  skip.onclick = () => { requested = true; finish(); };
  ...
  return {
    ...
    react(state) {
      ...
      status.textContent = { idle:'Ready', summoned:'Looking', thinking:'Investigating', verdict:'Answer ready', stumped:'No answer' }[state];
    },
    ...
    dispose() { finish(); clearTimeout(safety); animations.forEach(a => a.cancel()); if(heading)headingHome.insertBefore(heading,headingNext); style.remove(); status.remove(); delete document.body.dataset.seerState; document.body.classList.remove('seer-room'); document.removeEventListener('keydown', escape); },
  };
}
