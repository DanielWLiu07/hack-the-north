...
const errors=[];let missing=false, simulated=false;
try{
 for(const width of [1440,390]){
  ...
  page.on('request',r=>{const u=new URL(r.url()),p=u.pathname;const json=d=>r.respond({status:200,contentType:'application/json',body:JSON.stringify(d)});
   ...
   if(p==='/live/latest.json'&&simulated)return json({source:'robot',sender_mode:'sim',cameras:[{camera:'fake',color:'fake.jpg'}]});
   if(p==='/live/robot-map.json'&&missing)return r.respond({status:503,contentType:'application/json',body:JSON.stringify({detail:'Camera capture unavailable for this check.'})});
   r.continue();
  });
  ...
  for(const mode of ['cloud','voxels','map','captures','data']){
   ...
   if(mode==='cloud')assert(status.includes('HARDWARE MAP SNAPSHOT'));
   if(mode==='voxels')assert(status.includes('HARDWARE MAP SNAPSHOT'));
   if(mode==='map')assert(status.includes('HARDWARE MAP SNAPSHOT'));
   if(mode==='captures')assert(await page.$('.spatial-images img'));
   if(mode==='data')assert(status.includes('HARDWARE MAP SNAPSHOT'));
  ...
  missing=false;await page.click('.spatial-toolbar button:last-child');await page.waitForFunction(()=>document.querySelector('.spatial-status').textContent.includes('HARDWARE MAP SNAPSHOT'),{timeout:30000});
  simulated=true;await page.click('#perception-tab-captures');await page.click('.spatial-toolbar button:last-child');await page.waitForFunction(()=>document.querySelector('.spatial-status').textContent.includes('excluded'));assert.equal(await page.$('.spatial-images img'),null);simulated=false;
