...
| `GET /telemetry` | `telemetry_api.py` |
...
| `robot_view_api.py` (link session, [docs/33](../docs/33-robot-link.md)) | `GET /api/robot/view.mjpg` (multipart stream; an `<img>` plays it) · `/api/robot/view.jpg` (one frame; 503 `no_live_frame` with the reason) · `/api/robot/view/status`. Proxies the robot's `GET /camera/cam0.jpg` ([docs/16 §2.1b](../docs/16-api.md)): **every browser shares ONE poller**, and it stops 10 s after the last viewer leaves — a camera read costs the machine balancing the robot. Reads `PI_HOST` from the `.env` *file* at poll time, so `scripts/pi_link.py use …` needs no web restart. Touches neither ES nor room.git |
