...
| `capture_to_recording.py` | **The live robot → a perception recording** (docs/34): one gated `POST /capture` written as exactly what `perception/pipeline.py` reads — the robot's JPEG bytes, its calibration, and the **measured** mount (pitch 38.1° / 1.59 m, not bbos's 33° / 1.55 m). `--n N --every S` takes several and prints how well each agrees with the one before — on an untouched scene that table is the noise floor. `--compare A B` does the same for two existing recordings, no robot (`--json`). `--check-desk [DIR]`: is a desk where `room.yaml`'s zone expects it, and if not, what to move and by how much. Retries a 409 `capture_rejected` three times, never fakes one; a 403 = the robot's `ROBOT_ALLOW` does not list this laptop. |
| `room_live.py` | **The front door: capture → scan → git** (docs/34). `new <name>` makes an INSTANCE — a normal git repo under `~/.cache/gitspace/rooms/<name>/`, first capture, first commit; `status` / `commit -m` / `watch [--every S] [--commit]` each take a capture, scan it through the real pipeline (`ROOM_SCANNER=perception:<recording>`) and let `room status` / `room commit` do the rest; `log` · `diff` · `list`. Recordings (`<name>.recordings/`) and the coloured 3D model of every capture (`<name>.scene/*.ply` + `.png`, `latest.*`) sit BESIDE the repo, and the commit message carries the capture id. Instances run `ROOM_ES=off`; only `--repo ./room.git` publishes to Elasticsearch. The pose is not tracked: never move the robot within an instance. |
