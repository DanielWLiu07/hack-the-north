...
Every Elasticsearch mapping, write and query, plus the Agent Builder tool definitions.
...
| `tools.py` | Agent Builder tool definitions (retrieval side). |

## Build order
1. **Tonight:** Serverless project (Elasticsearch type — Agent Builder is on by default there),
...
6. `tools.py` — Agent Builder tools, because they're the feature the brief names.
7. Voxel ingest (T2).
