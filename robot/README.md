...
| `adapter.py` · `frames.py` | **built, simulated** | The edge's robot adapter on 127.0.0.1:8765 — `POINT_AT_OBJECT` first ([`RUNBOOK.md` §6b](RUNBOOK.md)). `frames.py` only re-exports `roomctl/frames.py`, the one room↔BB conversion, plus the registration's provenance. Hardware motion refused until Gate 1. |
| `probe_bbos.py` | built | Read-only probe of what bbos really publishes — depth **units measured against `camera.points`**, alignment, lag, intrinsics. Runs on the robot over ssh stdin, nothing deployed ([`RUNBOOK.md` §6](RUNBOOK.md)). |
